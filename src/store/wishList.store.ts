...
const wishListSlice = createSlice({
  ...
  reducers: {
    ...
    saveWishListItemsToLocalStorage: (state, action: PayloadAction<IProduct>) => {
      const isProductExistOnCart = state.products.some((product) => product.id == action.payload.id);
...
export const { getWishListItemsFromLocalStorage, clearWishList, saveWishListItemsToLocalStorage, removeWishListItem } =
